...
from basic_auth__SOLUTION import (
    b2i,
    blake2b_256,
    bytes_to_int,
    calc_A,
...
class RSA_key:
    ...
    def verify(self, message: Union[int, bytes], signature: Union[int, bytes]) -> bool:
...
def gen_plaintext(
    given_name: str, surname: str, birthdate: date, vax_count: int, last_vax_date: date
) -> bytes:
...
def interleave_data(plaintext: bytes, nonce: bytes, inner_tag: bytes) -> bytes:
    ...
    for _ in range(8):
        block = (
            nonce[NT_start:NT_end]
            + plaintext[P_start:P_end]
            + inner_tag[NT_start:NT_end]
        )
        data = data + block
        NT_start += 2
        NT_end += 2
...
def verify_passport(
    passport: bytes, key_enc: bytes, RSA_key: object, key_hash: Optional[bytes] = None
...
def request_passport(
    ...
    health_data = (
        int_to_bytes(health_id, 5)
        + bytes(3)
        + int_to_bytes(days, 2)
        + bytes(4)
        + int_to_bytes(vax_days, 2)
    )
...
def retrieve_passport(
    ...
    passport = create_passport(
        given_name, surname, birth_date, vax_count, vax_date, key_hash, key_enc, RSA_key
    )
...
if __name__ == "__main__":
    ...
    def QR_image_parser(string):
        ...
        try:
            ...
            raise ValueError(
                f"The image {string} either doesn't exist, or couldn't be read."
            )
        ...
        return int(results[0].data).to_bytes(
            319, "big"
        )  # TODO: use exceptions to catch longer codes
    ...
    cmdline = argparse.ArgumentParser(
        description="Test out the vaccine passport algorithms."
    )

    methods = cmdline.add_argument_group(
        "ACTIONS", "The four actions this program can do."
    )

    methods.add_argument("--request_QR", action="store_true", help="Request a QR code.")
    methods.add_argument(
        "--QR_server", action="store_true", help="Launch the demo server."
    )
    methods.add_argument(
        "--quit_server", action="store_true", help="Tell the QR server to quit."
    )
    methods.add_argument("--verify_QR", action="store_true", help="Verify a QR code.")

    methods = cmdline.add_argument_group(
        "PERSONAL", "Information about the person requesting the QR code."
    )
    ...
    if addr is not None:
        ...
        vax_database = {
            args.ohn: [args.given_name, args.surname, args.birth_date]
        ...
        def server_loop(IP, port, DH, RSA, key_hash, key_enc, bits, db, verbose=False):
            ...
            while True:
                ...
                if mode == b"q":
                    ...
                    temp = retrieve_passport(
                        client, DH, RSA, key_hash, key_enc, bits, registered, db
                    )
                    if (temp is None) and verbose:
                        printf(
                            f"Server: Retrieval failed, closing socket and waiting for another connection."
                        )
    ...
    if args.request_QR and (addr is not None):
        ...
        def client_routine(IP, port, verbose=False):
            ...
            if args.QR_output:
                image = qrcode.make(
                    b2i(passport)
                )  # storing it as a number is more reliable
        ...
        client_thread = Thread(
            target=client_routine, args=(IP, port, args.verbose), daemon=True
        )
        client_thread.start()
    ...
    if args.verify_QR:
        ...
        for input in [args.QRcode_file, args.QRcode_image, args.QRcode_webcam]:
            if input is not None:
                output = verify_passport(
                    input, args.key_enc, args.RSA_key, args.key_hash
                )
